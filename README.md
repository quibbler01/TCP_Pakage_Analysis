...
总结体会
	从这次实验中，不仅对计算机网络各层的消息结构有了清晰的了解，而且也学会了使用wireshark来获取网络数据的方法。在使用wireshark的过程中，结合自己在本课程所学到的网络知识，加深了自己对消息转发、协议处理、数据格式等的理解，不是在二进制的层面去看这些信息，而是结合获取的二进制数据对其进行分析，首先应该确定该消息使用的协议，因为只有判断正确这些截获二进制数据的协议类型，才能正确的知道消息转发所遵循的共同语法和语义，才能真正了解这个消息的含义。因此，我们小组在进行该实验的时候，首先对现有的消息协议进行了分析和分类，有利于我们写程序是根据什么进行判断消息的协议类型，确定消息类型之后，对消息的分析就根据它遵循的协议结构进行分析，就可以清晰的得到消息每部分数据是属于什么结构，这也是指导我们核心函数工作的主要思想。在这次实际编程时，也遇到了很多的问题，我们小组通过交流，通过理解网络上的解决办法，也都得到了解决。这次实验对我的影响也很大，学会了在团队中怎样发挥自己的作用，怎么让同组的人接受和采纳自己的建议，其实在工作中沟通才最重要，再次感谢同组的伙伴。
